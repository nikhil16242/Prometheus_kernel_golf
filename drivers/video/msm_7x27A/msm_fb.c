...
static void msmfb_early_resume(struct early_suspend *h)
{
	...
	if (!in_onchg_resume) {
		...
		/*
		 * set brightness to zero before leaving onchg mode,
		 * it can avoid the resume flash screen problem after onchg mode
		 */
		msm_fb_set_backlight(mfd, 0);
		printk(KERN_INFO "[DISP] %s: set brightness to zero before leaving onchg mode\n", __func__);
		hr_msleep(20);
	}
