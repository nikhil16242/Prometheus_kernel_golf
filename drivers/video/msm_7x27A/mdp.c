...
void mdp_pipe_kickoff(uint32 term, struct msm_fb_data_type *mfd)
{
	...
	if (term == MDP_PPP_TERM) {
		...
		ret = wait_for_completion_killable_timeout(&mdp_ppp_comp, 3* HZ);

		if (!ret) {
			pr_err("%s(%d)timeout but dma still busy\n", __func__, __LINE__);
			pr_err("MDP_DISPLAY_STATUS:%x\n", inpdw(msm_mdp_base + 0x38));
			pr_err("MDP_INTR_STATUS:%x\n", inpdw(MDP_INTR_STATUS));
			pr_err("MDP_INTR_ENABLE:%x\n", inpdw(MDP_INTR_ENABLE));
			pr_err("MDP_PRIM_INT_CNT_VAL:%x\n", inpdw(msm_mdp_base + 0x0340));
			pr_err("SOURCE_HW:%x\n",inpdw(msm_mdp_base + 0x10108));
			pr_err("OUT_HW:%x\n",inpdw(msm_mdp_base + 0x10164));
			pr_err("OUT_XY:%x\n",inpdw(msm_mdp_base + 0x1019C));
			pr_err("SOURCE_FORMAT:%x\n",inpdw(msm_mdp_base + 0x10124));
			pr_err("OUT_FORMAT:%x\n",inpdw(msm_mdp_base + 0x10150));
		}
